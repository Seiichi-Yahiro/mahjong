...
pub struct PlaceToolStateStagePlugin;

impl StateStagePlugin<Tool> for PlaceToolStateStagePlugin {
    fn build(&self, state_stage: &mut StateStage<Tool>) {
        let state = Tool::Place;
...
pub fn clean_up_system(commands: &mut Commands, query: Query<Entity, With<PlaceAbleTile>>) {
